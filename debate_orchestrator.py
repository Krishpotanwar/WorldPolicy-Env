...
class DebateOrchestrator:
    ...
    async def _call_groq(self, system_prompt: str, agent_id: str) -> dict:
        ...
        sanitized = {
            ...
            "mentioned_countries": self._map_countries(parsed.get("mentioned_countries", [])),
    ...
    def _map_countries(self, raw_list: list) -> list[str]:
        mapped = []
        for c in raw_list:
            if not isinstance(c, str): continue
            name = c.lower()
            if "united states" in name or "usa" in name or "america" in name: mapped.append("USA")
            elif "china" in name or "chn" in name: mapped.append("CHN")
            elif "russia" in name or "rus" in name: mapped.append("RUS")
            elif "india" in name or "ind" in name: mapped.append("IND")
            elif "korea" in name or "dprk" in name: mapped.append("DPRK")
            elif "saudi" in name or "sau" in name: mapped.append("SAU")
            elif "united nations" in name or "un " in name or name == "un": mapped.append("UN")
            elif "kingdom" in name or "gbr" in name or "britain" in name: mapped.append("GBR")
            elif "brazil" in name or "bra" in name: mapped.append("BRA")
            elif len(name) == 3: mapped.append(name.upper())
    ...
    def _get_canned(self, crisis_type: str, agent_order: list[str], round_num: int = 1) -> list[dict]:
        ...
        if crisis_type == "custom_scenario":
            return [{
                "speaker": a,
                "text": f"{a} reserves their position on this novel scenario while analyzing incoming data.",
                "stance": "neutral",
                "mentioned_countries": [],
                "authority_citation": None,
    ...
    async def run_debate_round(
        ...
        if use_live:
            ...
            timeout = GROQ_TIMEOUT + 2 if self._backend == "groq" else 15.0
            for agent_id in speaker_order:
                ...
                try:
                    use_groq = self._backend == "groq" or (crisis_type == "custom_scenario" and self._groq_client is not None)
                    if use_groq:
                        raw = await asyncio.wait_for(self._call_groq(prompt, agent_id), timeout=timeout)
                    else:
                        raw = await asyncio.wait_for(
                            self._call_hf_model(
                                ...
                                public_sentiment=sentiment,
                                round_num=1,
                                prior_utterances=round_utterances, # pass recent utterances
                            ),
                            timeout=timeout
                        )
                except Exception as e:
